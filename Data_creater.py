...
choice_list = [[1, 1], [1, 2], [1, 3], [2, 1], [2, 3], [3, 1], [3, 2]]
...
def generate_background(img, color = None, size=(960, 1280), nb_blobs=100, min_rad_ratio=0.01,
    ...
    dim = max(size)
    if color is None:
        img = cv2.randu(img, 0, 255)
    else:
        img = cv2.randu(img, 0 + int(color), 255)
    # cv2.threshold(img, randint(0, 256), 255, cv2.THRESH_BINARY, img)
    background_color = int(np.mean(img))
    blobs = []
    for i in range(nb_blobs):
        blobs.append([randint(0, size[1]), randint(0, size[0])])
    blobs = np.array(blobs)

    for i in range(nb_blobs):
        col = get_random_color(background_color)
        cv2.circle(img, (blobs[i][0], blobs[i][1]), randint(int(dim * min_rad_ratio), int(dim * max_rad_ratio)), col, -1)
                  # np.random.randint(int(dim * min_rad_ratio),
                  #                   int(dim * max_rad_ratio)),
                  # col, -1)
    kernel_size = randint(min_kernel_size, max_kernel_size)
...
def draw_circle(circle_mask, num):
    row, col = circle_mask.shape[:2]
    centre = []
    for i in range(num):
        mask = np.zeros((160, 160))
        rad = randint(20, 80)
        mask = cv2.circle(mask, (80, 80), rad, (255, 255, 255), -1)
        mask, M = rand_homography(mask)
        cen = M @ np.array((80, 80, 1)).T
        mask /= 255.
        if len(centre) == 0:
            x = randint(0, col - 170)
            y = randint(0, row - 170)
            circle_mask[y : y + 170, x : x + 170] = mask
            centre.append([x + int(cen[0]), y + int(cen[1])])
        else:
            while True:
                flag = True
                x = randint(0, col - 170)
                y = randint(0, row - 170)
                for each in centre:
                    if np.sqrt((each[0] - x - int(cen[0])) ** 2 + (each[1] - y - int(cen[1])) ** 2) < 400:
                        flag = False
                if flag:
                    break
            circle_mask[y : y + 170, x : x + 170] = mask
    ...
    return np.array(centre).astype(np.int)
...
def gaussian_noise(img, color=None):
    """ Apply random noise to the image """
    img = cv2.randu(img, 0, 255)
    img = cv2.blur(img, (10, 10), img)
...
def Gaussian_noise(img, color=None):
    ...
    num = 0
    while num < 45:
        kernel_size = (randint(20, 80)) * 2 + 1
        half = kernel_size // 2
        kernela = cv2.getGaussianKernel(kernel_size, kernel_size // 5)
        kernel = np.multiply(kernela, np.transpose(kernela))
        kernel /= kernel.max()

        x = randint(half + 1, img.shape[1] - half - 1)
        y = randint(half + 1, img.shape[0] - half - 1)
        rand_kernel = randint(80, 200) * kernel
        img[y-half:y+half+1,x-half:x+half+1] = np.max(np.array([img[y-half:y+half+1,x-half:x+half+1],rand_kernel]), axis=0)
        num += 1
    return np.empty((0, 2), dtype=np.int)
...
def rand_homography(img):
    img = img.copy()
    row, col = img.shape[:2]
    pts1 = np.float32([[10, 10], [100, 10], [10, 100]])
    pts2 = np.float32([[10, 10], [100, 10], [10 + (random() * 2 - 1) * 10, 100 + (random() * 2 - 1) * 10]])
    M = cv2.getAffineTransform(pts1, pts2)
    res = cv2.warpAffine(img, M, (col + 10, row + 10))
    return res, M

def random_color(img, color=None):
    clo = randint(30, 255)
    if color is not None:
        if abs(clo - color) < 30:  # not enough contrast
            clo = (color + 128) % 256
    img *= clo
    return img

def py_switch(arg):
    sw = {1:random_color, 2:generate_background, 3:gaussian_noise}

    return sw.get(arg, 'nothing')

def get_random_circle(nums):
    Img = []
    Center = []
    i = 0
    while True:
        circle_mask = np.zeros((960, 1280))
        num = randint(2, 5)
        center = draw_circle(circle_mask, num)
        # mode = [2, 1]
        mode = choice(choice_list)
        bk_mode = py_switch(mode[0])
        background = np.ones((960, 1280))
        foreground = np.ones((960, 1280))
        background = bk_mode(background)
        bk_clo = np.mean(background)
        ft_mode = py_switch(mode[1])
        ft_mode(foreground, bk_clo)

        background *= (1 - circle_mask)
        foreground *= circle_mask
        foreground += background

        # img_copy = foreground.copy()
        # img_copy = cv2.cvtColor(img_copy.astype(np.uint8), cv2.COLOR_GRAY2BGR)
        # for a, each in enumerate(center):
        #     img_copy = cv2.circle(img_copy, (int(each[0]), int(each[1])), 5, (0, 0, 255))
        #     img_copy = cv2.putText(img_copy, '{}'.format(a), (each[0], each[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
        # cv2.imshow('1', img_copy)
        # cv2.waitKey(400)

        # Img.append(foreground.astype(np.uint8))
    ...
    img_name = 'Data_folder/img/{}.npy'.format(nums)
    label_name = 'Data_folder/label/{}.pt'.format(nums)

    np.save(img_name, np.array(Img))
    with open(label_name, 'wb') as fp:
        pickle.dump(Center, fp)
    print("{} is ok".format(nums))

if __name__ == "__main__":
    # seed(0)
    for i in range(200):
        get_random_circle(i)
    # job_list = [a for a in range(100)]
    # pool = Pool(8)
    # pool.map_async(get_random_circle, job_list)
    # pool.close()
    # pool.join()
    # print('all jobs ok')
